...
class ProjectInvitationServiceTest {
  ...
  @InjectMocks private ProjectInvitationService projectInvitationService;
  ...
  void createShouldSaveInvitationAndNotifyKnownUser() {
    ...
    verify(activityLogService)
        .log(eq(project), eq(actor), eq("Created invitation for guest@example.com"));
  ...
  void acceptShouldCreateMemberAndMarkInvitationAccepted() {
    ...
    verify(activityLogService)
        .log(eq(project), eq(actor), eq("Accepted invitation and joined as MEMBER"));
    verify(notificationService)
        .notify(
            eq(inviter),
            any(),
