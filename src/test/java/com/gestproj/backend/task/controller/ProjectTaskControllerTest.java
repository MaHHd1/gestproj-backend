...
class ProjectTaskControllerTest {
    ...
    void createShouldReturnCreatedTask() {
        ...
        when(taskService.create(eq(1L), any(TaskCreateRequest.class), eq("owner@example.com"))).thenReturn(
                new TaskResponse(9L, 1L, "Task 1", "Description", TaskStatus.A_FAIRE, TaskPriority.MOYENNE, LocalDate.now(), false, null, null)
        );

        var response = projectTaskController.create(1L, new TaskCreateRequest("Task 1", "Description", TaskStatus.A_FAIRE, TaskPriority.MOYENNE, LocalDate.now(), null), authentication);
    ...
    void listShouldReturnTasks() {
        ...
        var response = projectTaskController.listByProject(1L, authentication, null, null, null, null, PageRequest.of(0, 20));
