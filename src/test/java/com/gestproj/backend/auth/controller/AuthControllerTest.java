...
class AuthControllerTest {
  ...
  void registerShouldReturnCreatedResponse() {
    when(authService.register(any(RegisterRequest.class)))
        .thenReturn(
            new AuthResponse(
                "token-123", new UserResponse(1L, "user@example.com", "user", "User Name", null)));

    var response =
        authController.register(
            new RegisterRequest("user@example.com", "user", "password123", "User Name"));
  ...
  void loginShouldReturnOkResponse() {
    when(authService.login(any(LoginRequest.class)))
        .thenReturn(
            new AuthResponse(
                "token-456", new UserResponse(1L, "user@example.com", "user", "User Name", null)));
