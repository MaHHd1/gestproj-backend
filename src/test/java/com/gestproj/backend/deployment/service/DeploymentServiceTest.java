...
class DeploymentServiceTest {
  ...
  void recordDeploymentShouldSaveAndReturnResponse() {
    ...
    var response =
        deploymentService.recordDeployment(
            5L,
            new DeploymentCreateRequest("SUCCESS", "abc123", "Deployed", "actor"),
            "actor@example.com");
    ...
    verify(activityLogService)
        .log(project, user, "Deployment succeeded: " + response.commitMessage());
  }
  ...
  void recordDeploymentShouldNotifyOnFailure() {
    ...
    var response2 =
        deploymentService.recordDeployment(
            6L,
            new DeploymentCreateRequest("FAILURE", "def456", "Bad deploy", "actor2"),
            "actor2@example.com");

    assertEquals(12L, response2.id());
    verify(activityLogService)
        .log(project2, actor2, "Deployment failed: " + response2.commitMessage());
