...
public class ProjectService {
  ...
  public ProjectResponse create(ProjectCreateRequest request, String currentUserEmail) {
    ...
    project.setDeploymentHost(null);
    project.setDeploymentContainer(null);
  ...
  public ProjectStatisticsResponse getStatistics(Long projectId, String currentUserEmail) {
    ...
    long completedTasks =
        allTasks.stream().filter(t -> t.getStatus() == TaskStatus.TERMINE).count();
    long inProgressTasks =
        allTasks.stream().filter(t -> t.getStatus() == TaskStatus.EN_COURS).count();
    long notStartedTasks =
        allTasks.stream().filter(t -> t.getStatus() == TaskStatus.A_FAIRE).count();
    long lateTasks =
        allTasks.stream().filter(com.gestproj.backend.task.entity.Task::isLate).count();

    double completionPercentage =
        totalTasks == 0 ? 0.0 : (double) completedTasks / totalTasks * 100;
  ...
  public ProjectResponse update(Long id, ProjectUpdateRequest request, String currentUserEmail) {
    ...
    project.setRepoOwner(request.repoOwner() == null ? null : request.repoOwner().trim());
    project.setRepoName(request.repoName() == null ? null : request.repoName().trim());
    project.setDeploymentHost(request.deploymentHost() == null ? null : request.deploymentHost().trim());
    project.setDeploymentContainer(request.deploymentContainer() == null ? null : request.deploymentContainer().trim());
    Project savedProject = projectRepository.save(project);
  ...
  private ProjectResponse toResponse(Project project) {
    return new ProjectResponse(
        ...
        project.getRepoName(), project.getDeploymentHost(), project.getDeploymentContainer());
  }
}
