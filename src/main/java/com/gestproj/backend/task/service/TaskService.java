...
public class TaskService {
    ...
    public TaskResponse create(Long projectId, TaskCreateRequest request, String actorEmail) {
        ...
        if (!actorMember.isCanCreateTask() && actorMember.getRole() != com.gestproj.backend.common.enums.ProjectMemberRole.OWNER) {
        ...
        activityLogService.log(project, userService.findEntityByEmail(actorEmail), "Created task " + savedTask.getTitle());
    ...
    public Page<TaskResponse> listByProject(
        ...
        if (Boolean.TRUE.equals(assignedToMe)) {
            specification = specification.and((root, query, cb) -> cb.equal(root.get("assignedTo").get("id"), actor.getId()));
        }
    ...
    public TaskResponse update(Long taskId, TaskUpdateRequest request, String actorEmail) {
        ...
        if (!actorMember.isCanEditTask() && actorMember.getRole() != com.gestproj.backend.common.enums.ProjectMemberRole.OWNER) {
        ...
        Task savedTask = taskRepository.save(task);
        activityLogService.log(project, userService.findEntityByEmail(actorEmail), "Updated task " + savedTask.getTitle());
        return toResponse(savedTask);
    }
    ...
    public void delete(Long taskId, String actorEmail) {
        ...
        if (!actorMember.isCanDeleteTask() && actorMember.getRole() != com.gestproj.backend.common.enums.ProjectMemberRole.OWNER) {
            throw new ForbiddenException("You are not allowed to delete tasks in this project");
        }

        activityLogService.log(task.getProject(), userService.findEntityByEmail(actorEmail), "Deleted task " + task.getTitle());
