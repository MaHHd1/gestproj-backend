...
public class ProjectMemberService {
  ...
  public ProjectMember addMember(Project project, User user, ProjectMemberRole role) {
    ...
    projectMember.setStatus(
        role == ProjectMemberRole.OWNER ? ProjectMemberStatus.ACTIVE : ProjectMemberStatus.INVITED);
    projectMember.setRoleTitle(role == ProjectMemberRole.OWNER ? "Owner" : "Member");
    projectMember.setRoleDescription(
        role == ProjectMemberRole.OWNER ? "Project owner" : "Project member");
  ...
  public boolean isMember(Project project, User user) {
    return projectMemberRepository.existsByProjectAndUserAndStatus(
        project, user, ProjectMemberStatus.ACTIVE);
  }
  ...
  public ProjectMemberResponse updateMember(
      Project project, Long memberId, ProjectMemberUpdateRequest request, User actor) {
    ProjectMember actorMember =
    ...
    if (member.getRole() == ProjectMemberRole.OWNER
        && request.role() != null
        && request.role() != ProjectMemberRole.OWNER) {
  ...
  public void removeMember(Project project, Long memberId, User actor) {
    ProjectMember actorMember =
        projectMemberRepository
            ...
            .orElseThrow(() -> new ForbiddenException("You are not a member of this project"));
    if (actorMember.getStatus() != ProjectMemberStatus.ACTIVE
        || (actorMember.getRole() != ProjectMemberRole.OWNER
            && !actorMember.isCanManageMembers())) {
      throw new ForbiddenException("You are not allowed to manage project members");
    }
  ...
  public ProjectMember findProjectMember(Project project, User user) {
    return projectMemberRepository
        .findByProjectAndUserAndStatus(project, user, ProjectMemberStatus.ACTIVE)
        .orElseThrow(() -> new ResourceNotFoundException("Project member not found"));
  }
