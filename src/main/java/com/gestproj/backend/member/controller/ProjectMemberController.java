...
public class ProjectMemberController {
  ...
  public ResponseEntity<ProjectMemberResponse> update(
    ...
    return ResponseEntity.ok(
        projectMemberService.updateMember(project, memberId, request, currentUser));
  }

  @DeleteMapping("/{memberId}")
  public ResponseEntity<Void> remove(
      @PathVariable Long projectId, @PathVariable Long memberId, Authentication authentication) {
    Project project = getProject(projectId);
    User currentUser = userService.findEntityByEmail(authentication.getName());
  ...
  private void ensureMember(Project project, User user) {
    ...
    if (member == null) {
      throw new com.gestproj.backend.common.exception.ForbiddenException(
