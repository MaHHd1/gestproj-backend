...
public class ProjectInvitationService {
  ...
  public ProjectInvitationResponse create(
      Long projectId, ProjectInvitationCreateRequest request, String actorEmail) {
    ...
    String invitedEmail =
        request.invitedEmail() == null
            ? null
            : request.invitedEmail().trim().toLowerCase(Locale.ROOT);
    ...
    invitation.setProposedRole(
        request.proposedRole() == null ? ProjectMemberRole.MEMBER : request.proposedRole());
    invitation.setRoleTitle(request.roleTitle() == null ? "Member" : request.roleTitle().trim());
    invitation.setRoleDescription(
        request.roleDescription() == null ? "Project member" : request.roleDescription().trim());
    ...
    activityLogService.log(
        project, actor, "Created invitation for " + (invitedEmail == null ? "link" : invitedEmail));
    if (invitedEmail != null && userRepository.findByEmail(invitedEmail).isPresent()) {
  ...
  public List<ProjectInvitationResponse> listForProject(Long projectId, String actorEmail) {
    ...
    return projectInvitationRepository.findAllByProjectOrderByCreatedAtDesc(project).stream()
        .map(this::toResponse)
        .toList();
  }
  ...
  public ProjectInvitationResponse accept(String token, String actorEmail) {
    ...
    if (projectMemberRepository.existsByProjectAndUser(project, actor)) {
      ProjectMember existingMember =
          projectMemberRepository.findByProjectAndUser(project, actor).orElseThrow();
    ...
    ProjectInvitation savedInvitation = projectInvitationRepository.save(invitation);
    activityLogService.log(
        project, actor, "Accepted invitation and joined as " + savedMember.getRole());
    notificationService.notify(
        invitation.getInvitedBy(),
        NotificationType.INVITATION_ACCEPTED,
        "Invitation accepted",
  ...
  public ProjectInvitationResponse getByToken(String token, String actorEmail) {
    ...
    boolean isProjectMember =
        projectMemberRepository.existsByProjectAndUser(invitation.getProject(), actor);
